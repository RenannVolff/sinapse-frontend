    ...
    <div className="max-w-6xl mx-auto space-y-6 fade-in pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Últimas 50 ações realizadas na sua conta.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center p-16">
            ...
            <h3 className="text-xl font-bold text-gray-900">Nenhuma ação registrada</h3>
            <p className="text-gray-500 mt-2">Assim que você criar, editar ou excluir algo, o histórico aparecerá aqui.</p>
          </div>
        ) : (
                ...
                {logs.map((log) => {
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="p-5">
                        <span className={`px-3 py-1 text-xs font-bold rounded-full border ${ACAO_BADGE[log.acao] ?? 'bg-gray-50 text-gray-700 border-gray-100'}`}>
                          {ACAO_LABEL[log.acao] ?? log.acao}
                        </span>
                      </td>
                      <td className="p-5 text-gray-700 font-medium">{log.recurso}</td>
                      <td className="p-5 text-gray-600">{log.resumo}</td>
                    </tr>
                  );
                })}
              </tbody>
